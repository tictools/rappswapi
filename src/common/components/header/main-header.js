...
const MainHeader = ({ title, ...router }) => {
  const pathname =
    (router && router.location && router.location.pathname) || ROUTES.HOME;
  ...
  return (
    <div className={styles["main-header__container"]}>
      ...
      {!isHomePath && (
        <nav className={styles["main-header__nav"]}>{navBarContent}</nav>
      )}
    </div>
  );
};
...
const getNavBarContent = (pathname) => {
  return pathname === ROUTES.CATEGORIES ? (
    <div className={styles["main-header__link-container"]}>
      <Link className={styles["main-header__link"]} to={ROUTES.HOME}>
        Home
      </Link>
    </div>
  ) : (
    <div className={styles["main-header__link-container"]}>
      ...
      <Link className={styles["main-header__link"]} to={ROUTES.CATEGORIES}>
        Categories
      </Link>
    </div>
  );
